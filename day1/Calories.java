...
public class Calories {
    ...
    */
    public static int solve_part_1(List<List<Integer>> input) {
        int max_sum = 0;
        for(List<Integer> current_list : input) {
            int sum = 0;
            for(int next : current_list) {
                sum += next;
            }
    ...
    */
    public static int solve_part_2(List<List<Integer>> input) {
        PriorityQueue<Integer> sums = new PriorityQueue<Integer>(Collections.reverseOrder());
        for(List<Integer> current_list : input) {
            int sum = 0;
            for(int next : current_list) {
                sum += next;
        ...
        return sums.poll() + sums.poll() + sums.poll();
    }

    public static List<List<Integer>> read_input() {
        List<List<Integer>> input_lists = new ArrayList<List<Integer>>();
        try {
            ...
            while(scanner.hasNextLine()) {
                ...
                Scanner line_scanner = new Scanner(next_line);
                if(line_scanner.hasNextInt()) {
                    current_list.add(line_scanner.nextInt());
                } else {
                    input_lists.add(current_list);
