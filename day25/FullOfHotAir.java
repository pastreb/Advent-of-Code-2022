...
public class FullOfHotAir {
  ...
  hot air balloons drift down to meet you. Crews quickly start unloading the 
  ...
  those conditions! There are only a few places in the-- where did you say 
  you are again?"
  ...
  */
  ...
  public static String solve_part_1(List<String> input) {
    long sum = 0;
    for(String number : input) {
      sum += translate_from_snafu(number);
    }
    return translate_to_snafu(sum);
  }

  public static long translate_from_snafu(String snafu_number) {
    long number = 0;
    long digit = 1;
    for(int i = snafu_number.length()-1; i >= 0; i--) {
      String next_snafu_digit = snafu_number.substring(i, i+1);
      if(next_snafu_digit.equals("=")) {
        number += digit * (-2);
      } else if(next_snafu_digit.equals("-")) {
        number += digit * (-1);
      } else if(next_snafu_digit.equals("1")) {
        number += digit;
      } else if(next_snafu_digit.equals("2")) {
        number += digit * 2;
      }
      // There is no need to treat the 0 case
      digit *= 5;
    }
    // System.out.println(snafu_number + " becomes " + number);
    return number;
  }

  public static String translate_to_snafu(long number) {
    if(number == 0) {
      return "";
    }
