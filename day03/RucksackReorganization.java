    ...
    */
    ...
    public static int solve_part_1(List<String> input) {
        ...
        for(String rucksack : input) {
            ...
            for(int i = length/2; i < length; i++) { // iterate through second compartment
                char x = rucksack.charAt(i);
                if(rucksack.indexOf(x) < length/2) { // check if in first compartment
                    if(Character.isUpperCase(x)) {
                        sum += ((int) x)-38; // map ASCII 65 to 27
                    } else {
                        sum += ((int) x)-96; // map ASCII 97 to 1
                    }
                    break;
                }
            }
    ...
    */
    ...
    public static int solve_part_2(List<String> input) {
        ...
        for(int i = 0; i < input.size(); i+=3) {
            String elf_1 = input.get(i);
            String elf_2 = input.get(i+1);
            String elf_3 = input.get(i+2);
            if(elf_1.length() > elf_2.length()) {
                String temp = elf_1;
                elf_1 = elf_2;
                elf_2 = temp;
            }
            if(elf_1.length() > elf_3.length()) {
                String temp = elf_1;
                elf_1 = elf_3;
                elf_3 = temp;
            }
            // At this point elf_1 has the least amount of characters
            for(int j = 0; j < elf_1.length(); j++) {
                char x = elf_1.charAt(j);
                if(elf_2.indexOf(x) > -1 && elf_3.indexOf(x) > -1) { // check if in other elve's rucksacks
                    if(Character.isUpperCase(x)) {
                        sum += ((int) x)-38; // map ASCII 65 to 27
                    } else {
                        sum += ((int) x)-96; // map ASCII 97 to 1
