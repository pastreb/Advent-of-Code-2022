...
public class NoSpaceDevice {
    ...
    */
    ...
    public static int solve_part_1(int[][] input) {
        ...
        for(int i = 0; i < height; i++) { // go through rows
            ...
            curr_max = -1;
            for(int j = width-1; j >= 0; j--) { // from right to left
                if(input[i][j] > curr_max) {
                    curr_max = input[i][j];
                    visibility[i][j]++;
                }
        ...
        for(int i = 0; i < width; i++) { // go through columns
            int curr_max = -1;
            for(int j = 0; j < height; j++) { // from top down
            ...
            curr_max = -1;
            for(int j = height-1; j >= 0; j--) { // from button up
                if(input[j][i] > curr_max) {
                    curr_max = input[j][i];
                    visibility[j][i]++;
                }
        ...
        // And finally, count the ones that are visible:
        int n_visible = 0;
    ...
    */
    ...
    public static int solve_part_2(int[][] input) {
        int width = input[0].length;
        int height = input.length;
        int max_score = 0;
        for(int i = 0; i < height; i++) {
            for(int j = 0; j < width; j++) {
                ...
                if(i == 0 || j == 0 || i == height-1 || j == width-1) {
                    // There is at least one factor 0 in the score computation
                    continue;
                }
                int score = 1;
                // (1) Look up:
                int dist = 1;
                while(i-dist >= 1 && input[i-dist][j] < current_tree) {
                    dist++;
                }
                score *= dist;
                // (2) Look down:
                dist = 1;
                while(i+dist < height-1 && input[i+dist][j] < current_tree) {
                    dist++;
                }
                score *= dist;
                // (3) Look left:
                dist = 1;
                while(j-dist >= 1 && input[i][j-dist] < current_tree) {
                    dist++;
                }
                score *= dist;
                // (4) Look right:
                dist = 1;
                while(j+dist < width-1 && input[i][j+dist] < current_tree) {
                    dist++;
                }
                score *= dist;
                // Possibly update max score:
    ...
    public static int[][] read_input() {
        ...
        int width = input_list.get(1).length();
        int height = input_list.size();
        int[][] input_array = new int[width][height];
        for(int i = 0; i < height; i++) {
            String current_row = input_list.get(i);
            for(int j = 0; j < width; j++) {
                input_array[i][j] = Integer.parseInt(current_row.substring(j, j+1));
            }
        }
        return input_array;
    }
